...
below we fit a model with various link functions. For this model, however, the correct link function to use is the log-log link. From these models, weconstruct Q-Q plots of the residuals using $R = 100$ bootstrap replicates. The results are displayed in Figure~\ref{fig:link}.
\begin{example}
...
\end{example}
From the Q-Q plots in Figure~\ref{fig:link} it is clear the the model with the log-log link (which corresponds to gumber errors in the latent variable formulation) is the most appropriate, while the other plots indicate deviations from the hypothesized model.

\begin{figure}[!htbp]
  \centering
  \includegraphics[width=1\textwidth]{link}
...
An important feature of the cumulative link model \eqref{eqn:clm} is the proportional odds assumption, which assumes that mean structure $f\left(\boldsymbol{X}, \boldsymbol{\beta}\right)$, remain the same for each of the $J$ categories. \citet[pp. 334--335]{regression-harrell-2001} suggests computing each observation's contribution to the first derivative of the log likelihood function with respect to $\boldsymbol{\beta}$, average them within each of the $J$ categories, and examine any trends in the residual plots, but these plots can be difficult to intepret. FOrtunately, it is relatively straight forward to use the simulated surrogate response values $\mathcal{S}$ to check the proportionality assumption.

To illustrate, we generated 2000 observations from each of the following two probit models
\begin{equation*}
  Pr\left(\mathcal{Y} \le j\right) = \Phi\left(\alpha_j + \beta_1 X\right) \quad and \quad Pr\left(\mathcal{Y} \le j\right) = \Phi\left(\alpha_j + \beta_2 X\right), \quad j = 1, 2, 3,
\end{equation*}
...
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=1\textwidth]{proportionality}
