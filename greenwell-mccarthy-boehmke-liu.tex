...
\end{itemize}
In addition, the package also includes five simulated data sets: \code{df1}, \code{df2}, \code{df3}, \code{df4}, and \code{df5}. These data sets are used throughout the paper to demonstrate how the surrogate residual can be useful as a diagnostic tool for ordinal regression models. The R code used to generate these data sets is available on the projects GitHub page: \url{https://github.com/AFIT-R/sure/blob/master/data-raw/data.R}.
...
As discussed in Section~\ref{sec:surrogate}, one of the properties of the surrogate residual $R_S$ is that, if the model is specified correctly, then $Var\left(R_S | X\right) = c$, where $c$ is a constant.
...
The following block of code uses the \code{orm} function from the popular \pkg{rms} package to fit a probit model to the simulated data. \strong{Note:} we had to set \code{x = TRUE} in the call to \code{orm} in order to use the \code{presid} function later.
\begin{example}
# Fit a cumulative link model with probit link
...
\end{equation*}
where $\alpha_1 = -1.5$, $\alpha_2 = 0$, $\alpha_3 = 1$, $\alpha_4 = 3$, $\beta_1 = 1$, and $\beta_2 = 1.5$. The data are available in the data frame \code{df4} within the package; see \code{?df4} for details. 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Detecting interaction effects}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\end{equation*}
where $\alpha_1 = -16$, $\alpha_2 = -12$, $\alpha_3 = -8$, $\beta_1 = 8$, $\beta_2 = 3$, $\beta_3 = 5$, $x_1 \sim \mathcal{U}\left(1, 7\right)$, and $x_2$ is a factor with levels \code{Treatment} and \code{Control}. The simulated data are available in the \code{df5} data frame loaded with the \pkg{sure} package; see \code{?df5} for details. Below, we fit two probit models using the \code{clm} function from the \pkg{ordinal} package (which should already be loaded). The first model only corresponds to the simulated control group, while the second model corresponds to the treatment group.
\begin{example}
